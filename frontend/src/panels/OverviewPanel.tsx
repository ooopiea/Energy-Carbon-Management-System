...
export function OverviewPanel() {
  ...
  const apply = async () => {
    ...
    setBusy(true)
    try { await submitControlAction({ system: 'overview', action: '应用园区策略', target: 'demand_cap_kw', value: 0, unit: 'kW', reason: `优化目标=${strategy}`, actor: 'engineer' }) } catch { /* store 已展示服务端错误 */ } finally { setBusy(false) }
  }
  const undo = async () => {
  ...
  return <div className="panel-stack">
    ...
    <ActionHistory />
    <Card title="园区控制策略" icon={<SlidersHorizontal className="icon-sm industrial" />}><div className="form-grid"><label>优化目标<select value={strategy} onChange={e => setStrategy(e.target.value)}><option value="cost">电费最优</option><option value="carbon">低碳优先</option><option value="weighted">电费+碳排加权</option></select></label><div className="button-row"><button disabled={busy} className="btn primary" onClick={apply}>{busy ? '提交中…' : '确认应用'}</button><button disabled={busy} className="btn" onClick={undo}>撤销</button></div></div></Card>
  </div>
}
