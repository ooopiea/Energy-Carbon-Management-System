...
export function OverviewPanel() {
  ...
  const [cap, setCap] = useState('5200')
  ...
  return <div className="panel-stack">
    ...
    <Card title="园区控制策略" icon={<SlidersHorizontal className="icon-sm industrial" />}><div className="form-grid"><label>优化目标<select value={strategy} onChange={e => setStrategy(e.target.value)}><option value="cost">电费最优</option><option value="carbon">低碳优先</option><option value="demand">需量抑制</option></select></label><label>需量上限（kW）<input type="number" min="1000" max="20000" value={cap} onChange={e => setCap(e.target.value)} /></label><div className="button-row"><button disabled={busy} className="btn primary" onClick={apply}>{busy ? '提交中…' : '确认应用'}</button><button disabled={busy} className="btn" onClick={undo}>撤销</button></div></div></Card>
    <ActionHistory />
  </div>
}
