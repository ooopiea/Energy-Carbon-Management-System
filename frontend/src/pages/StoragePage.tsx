...
export function StoragePage() {
  ...
  return <div className="page-stack">
    <div className="page-title-row"><div><h1>储能系统</h1><p>30 MWh / 15 MW 电池系统健康、日前计划与实时响应</p></div></div>
    ...
    <Card title="储能设备拓扑" icon={<Boxes className="icon-sm industrial" />}><div className="storage-topology" aria-label="储能系统设备拓扑">
      ...
      <TopologyDevice id="pcs" label="PCS 变流器" sub="15 MW" active={contextSelection?.id === 'pcs'} onClick={() => selectDevice('pcs', 'PCS 变流器')} />
      <span className="topology-link" />
      <TopologyDevice id="battery-cabinet" label="电池舱" sub="6 簇 · 30 MWh" active={contextSelection?.id === 'battery-cabinet'} onClick={() => selectDevice('battery-cabinet', '储能电池舱')} />
      <div className="cluster-row">{[1,2,3,4,5,6].map(n => <button className={contextSelection?.id === `cluster-${n}` ? 'active' : ''} key={n} onClick={() => selectDevice(`cluster-${n}`, `电池簇 ${n}`)}><Battery /><span>簇 {n}</span><small>{(state.storage_soc * 100 + (n - 3.5) * .25).toFixed(1)}%</small></button>)}</div>
    </div></Card>
    <div className="two-col">
      <Card title="SOC 计划与实时反馈" icon={<Battery className="icon-sm industrial" />}><ReactECharts option={{ ...chartBase, legend: { top: 5, textStyle: { fontSize: 10 } }, yAxis: { ...chartBase.yAxis, name: 'SOC %', min: 0, max: 100 }, series: [{ name: '日前计划', type: 'line', symbol: 'none', data: state.day_ahead.soc_plan.map(v => v * 100), lineStyle: { color: '#8da1a6', type: 'dashed' } }, { name: '实时反馈', type: 'line', connectNulls: false, symbol: 'circle', symbolSize: 3, data: actual('soc'), lineStyle: { color: '#15803d', width: 2 }, areaStyle: { color: 'rgba(21,128,61,.08)' } }] }} onEvents={{ click: selectPoint }} style={{ height: 260 }} /></Card>
      <Card title="充放电计划与实时功率" icon={<Zap className="icon-sm industrial" />}><ReactECharts option={{ ...chartBase, legend: { top: 5, textStyle: { fontSize: 10 } }, yAxis: { ...chartBase.yAxis, name: 'kW' }, series: [{ name: '日前计划', type: 'line', symbol: 'none', data: state.day_ahead.storage_plan, lineStyle: { color: '#8da1a6', type: 'dashed' } }, { name: '实时功率', type: 'bar', data: actual('storage_power'), itemStyle: { color: (p: any) => p.value >= 0 ? '#15803d' : '#176b87' } }] }} onEvents={{ click: selectPoint }} style={{ height: 260 }} /></Card>
    </div>
    <Card title="电芯实时温度与安全边界" icon={<Thermometer className="icon-sm industrial" />}><ReactECharts option={{ ...chartBase, yAxis: { ...chartBase.yAxis, name: '°C' }, series: [{ name: '日前温度估计', type: 'line', symbol: 'none', data: ss?.temp_c || [], lineStyle: { color: '#9ca3af', type: 'dashed' } }, { name: '实时温度', type: 'line', connectNulls: false, symbol: 'circle', symbolSize: 3, data: actual('temp'), lineStyle: { color: '#c8493d', width: 2 }, markLine: { silent: true, data: [{ yAxis: 45, label: { formatter: '安全上限' }, lineStyle: { color: '#dc2626', type: 'dashed' } }] } }] }} onEvents={{ click: selectPoint }} style={{ height: 230 }} /></Card>
