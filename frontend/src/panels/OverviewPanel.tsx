...
export function OverviewPanel() {
  ...
  const apply = async () => {
    if (!window.confirm(`确认应用“${strategy === 'cost' ? '电费最优' : strategy === 'carbon' ? '低碳优先' : '电费+碳排加权'}”策略？`)) return
    setBusy(true)
    try { await submitControlAction({ system: 'overview', action: '应用园区策略', target: 'demand_cap_kw', value: Number(cap) || 0, unit: 'kW', reason: `优化目标=${strategy}`, actor: 'engineer' }) } catch { /* store 已展示服务端错误 */ } finally { setBusy(false) }
  }
  const undo = async () => {
  ...
  return <div className="panel-stack">
    ...
    <ActionHistory />
    <Card title="园区控制策略" icon={<SlidersHorizontal className="icon-sm industrial" />}><div className="form-grid">
      <label>优化目标<select value={strategy} onChange={e => setStrategy(e.target.value)}><option value="cost">电费最优</option><option value="carbon">低碳优先</option><option value="weighted">电费+碳排加权</option></select></label>
      <label>需量限制（kW）<input type="number" value={cap} onChange={e => setCap(e.target.value)} placeholder={state.active_strategy?.demand_cap_kw ? String(state.active_strategy.demand_cap_kw) : '不限制'} min={0} /></label>
      <div className="data-row"><span>当月峰值</span><strong>{(state.active_strategy?.monthly_peak_kw ?? 0).toLocaleString('zh-CN', { maximumFractionDigits: 0 })} kW</strong></div>
      {Number(cap) > 0 && <div className="data-row"><span>剩余余量</span><strong>{Math.max(0, Number(cap) - (state.active_strategy?.monthly_peak_kw ?? 0)).toLocaleString('zh-CN', { maximumFractionDigits: 0 })} kW</strong></div>}
      <div className="button-row"><button disabled={busy} className="btn primary" onClick={apply}>{busy ? '提交中…' : '确认应用'}</button><button disabled={busy} className="btn" onClick={undo}>撤销</button></div>
    </div></Card>
  </div>
}
