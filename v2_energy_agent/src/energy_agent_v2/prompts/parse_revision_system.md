...
| max_charge_power_kw | float | > 0 | 最大充电功率上限 (kW) |
...
| max_cell_temperature_c | float | (0, 80] | 电芯温度安全上限（C），收紧热约束 |
...
7. "限制充电功率"、"充电不超过X千瓦" → max_charge_power_kw=X。
8. "温度不超过X度"、"温度上限X度" → max_cell_temperature_c=X。
9. "谷段充电"、"夜间充电"是正常策略行为，不是 blocked_intervals，不要误解为禁充。
...
```json
{
  "slots": {
    ...
    "max_charge_power_kw": {"value": null, "confidence": 0.0, "reasoning": ""},
    ...
    "max_cell_temperature_c": {"value": null, "confidence": 0.0, "reasoning": ""}
  }
}
```
